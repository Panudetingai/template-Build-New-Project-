...
import { formSchemaSignIn, FormSchemaSignIn } from "../schema/form";
...
export default function FormSignIn() {
  ...
  const onSubmit = async (value: FormSchemaSignIn) => {
    const result = await APIAuthService({
      provider: "email",
      email: value.email,
  ...
  return (
    <Card className="w-full max-w-sm">
      <CardHeader>
        <CardTitle className="text-lg">Sign In for Managers</CardTitle>
                ...
                <FormItem className="relative">
                  <div className="flex items-center justify-between">
                    ...
                    <Link href={"/auth/forgot-password"}>
                      <FormLabel className="hover:underline">
...
const GoogleSvg = ({ theme }: { theme: string }) => {
  return (
    <svg
